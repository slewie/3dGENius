...
class GraphGAN(nn.Module):
    ...
    def __init__(self, z_size=100, num_vertex=100, lr=0.001, beta1=0.9, beta2=0.999, device='cpu',
                 criterion=nn.BCELoss(), smooth=False, generator=None, discriminator=None):
    ...
    def _discriminator_part(self, real_images, input_size):
        ...
        z = torch.FloatTensor(input_size, self.z_size).uniform_(-1, 1).to(self.device)
        fake_images = self.generator(z)

        D_fake = self.discriminator(fake_images)
        return self._fake_loss(D_fake, input_size, self.smooth) + self._real_loss(D_real, input_size, self.smooth)
    ...
    def _generator_part(self, input_size):
        ...
        D_fake = self.discriminator(fake_images)
        return self._real_loss(D_fake, input_size, self.smooth)
    ...
    def _get_symmetric_adjacency_matrix(self, tensor):
        ...
        for i in range(tensor.shape[0]):
            for j in range(tensor.shape[1]):
                output[i, j] = max(tensor[i, j], tensor[j, i])
        return output
    ...
    def generate_adj(self, return_symmetric=False):
        ...
        """
        self.generator.eval()
        z = torch.FloatTensor(1, self.z_size).uniform_(-1, 1).to(self.device)
        adjacency_matrix = torch.round(self.generator(z)).reshape(self.num_vertex, self.num_vertex)
        if return_symmetric:
    ...
    def generate_feat(self):
        self.generator.eval()
        z = torch.FloatTensor(1, self.z_size).uniform_(-1, 1).to(self.device)
        feature_matrix = self.generator(z).view(-1, self.num_vertex, self.generator.num_features)
